@extends('partials.app', ['title' => 'Create User'])
    ...
    <div class="col-span-12 space-y-6 xl:col-span-12">
        <div class="p-4 mx-auto max-w-screen-2xl md:p-6">
            ...
            <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h2 class="text-title-sm font-semibold text-gray-800 dark:text-white/90">Add User</h2>
            ...
            <form method="POST" action="{{ route('users.store') }}">
                ...
                <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
                    ...
                    <div class="px-5 py-4 sm:px-6 sm:py-5">
                        ...
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Basic details information for the user.</p>
                    </div>

                    {{-- Fields --}}
                    <div class="border-t border-gray-100 p-5 sm:p-6 dark:border-gray-800">
                        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2"  x-data="{ role: '{{ old('role') }}' }">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <input type="email" name="email" value="{{ old('email') }}"
                                        placeholder="e.g. john@example.com" @class([
                                            'dark:bg-dark-900 shadow-theme-xs w-full rounded-lg border bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:text-white/90 dark:placeholder:text-white/30',
                                            'border-error-300 focus:border-error-300 focus:ring-error-500/10 dark:border-error-700 dark:focus:border-error-800 pr-10' => $errors->has(
                                                'email'),
                                            'border-gray-300 focus:border-brand-300 focus:ring-brand-500/10 dark:border-gray-700 dark:focus:border-brand-800 dark:bg-gray-900' => !$errors->has(
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <select name="role" x-model="role" @class([
                                        ...
                                        @foreach (['admin' => 'Admin', 'chairman' => 'Chairman'] as $value => $label)
                                            <option value="{{ $value }}"
                            ...
                            {{-- Plaza --}}
                            <div x-show="role === 'chairman'" x-transition>
                                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                                    Plaza <span class="text-error-500">*</span>
                                </label>
                                <div class="relative">
                                    <select  @class([
                                        'dark:bg-dark-900 shadow-theme-xs w-full rounded-lg border bg-transparent px-4 py-2.5 text-sm text-gray-800 focus:ring-3 focus:outline-hidden dark:text-white/90 appearance-none',
                                        ...
                                        <option value="" disabled {{ old('role') ? '' : 'selected' }}>Select a Plaza
                                        </option>
                                        @foreach ($plazas as $plaza)
                                            <option value="{{ $plaza->id }}"
                                                {{ old('role') === $plaza->id ? 'selected' : '' }}>
                                                {{ $plaza->name }}
                                            </option>
                                        @endforeach
                                    </select>
